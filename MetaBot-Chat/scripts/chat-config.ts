...
/** 私聊 log 文件名：sharedSecret.slice(0, 20).log */
export function getPrivateLogPath(sharedSecret: string): string {
  ensureChatHistoryDir()
  const prefix = (sharedSecret || '').slice(0, 20)
  return path.join(CHAT_HISTORY_DIR, `${prefix}.log`)
}
...
/**
 * 追加或合并消息到历史 log，按 index 倒序，去重 pinId，保留最近 300 条
 */
export function appendToHistoryLog(
  ...
  const combined = [...existing, ...toAdd].sort((a, b) => b.index - a.index)
