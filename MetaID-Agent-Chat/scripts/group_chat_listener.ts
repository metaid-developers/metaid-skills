 ...
 * - 未指定 Agent：不在群内发消息，仅在控制台打印「群聊监听已确认开启，接下来你可以指定某个 MetaID-Agent 参与到群聊讨论中」
 */
async function runHandshake(
  ...
  if (!specifiedAgent) {
    console.log('\n🤝 群聊监听已确认开启，接下来你可以指定某个 MetaID-Agent 参与到群聊讨论中')
    return
  }
...
async function main() {
  ...
  console.log('🔄 MetaID-Agent 群聊监听已启动')
