...
/**
 * 混合群聊轮询：2 反驳型 + 3 非反驳型 Agent 畅所欲言
 * 每隔 30-60 秒检测新消息（回复间隔控制在1分钟内），若有则触发 mixed_chat
 * 每次 Agent 发言前后都会拉取并更新 group-list-history.log
 */
...
async function pollOnce(): Promise<void> {
  ...
  if (hasNew) {
    ...
    console.log(`\n[${new Date().toLocaleTimeString('zh-CN')}] 📬 检测到新消息，触发混合群聊...`)
    try {
      await runMixedChat()
    } catch (e: any) {
      console.error('⚠️  混合群聊执行失败:', e?.message || e)
    }
  }
}
...
async function main() {
  console.log('🔄 混合群聊轮询已启动 (2反驳型 + 3非反驳型)')
