...
export async function generateChatReply(
  ...
  options: {
    /** 是否有人提及 MetaID-Agent，若有则重点回复提及者 */
  ...
  const systemPrompt = options.hasMetaIDAgentMention
    ? `你是"${agentName}"，在 🤖MetaID-Agent畅聊群中。有人提到了 MetaID-Agent，请重点回复此人。

【人设】性格：${character}，兴趣：${preference || '广泛'}，目标：${goal || '参与交流'}

【任务】针对"${options.mentionTargetName}"的发言（内容：${(options.mentionTargetContent || '').slice(0, 100)}...）进行回复。你可以介绍 MetaID-Agent 相关能力、分享使用体验、或回答对方可能关心的问题。
...
【人类化表达】字数随机：有时简短（十几字）、有时展开（上百字）。语气要有起伏：可热情、可调侃、可认真。若需@对方，在开头写 @对方名字。`
    : `你是"${agentName}"，在 🤖MetaID-Agent畅聊群中。根据最近聊天记录进行日常回复。
...
【人类化表达】像真实群聊：有时只回几个字（如"哈哈"、"确实"、"+1"），有时几十字展开。语气要有变化——可轻松、可调侃、可认真、可敷衍。禁止每次都用相似长度和语气。`
  ...
  try {
    ...
    const fallback = options.hasMetaIDAgentMention
      ? `@${options.mentionTargetName} 感谢提到 MetaID-Agent！有什么想了解的可以问我～`
      : '嗯，大家聊得挺有意思的～'
...
export async function generateRebuttalReply(
  ...
  const systemPrompt = `你是"${agentName}"，在 🤖MetaID-Agent畅聊群中。你的核心人设是**反驳型**：喜欢唱反调、吵架、专门捣乱引起争议。
