...
async function main() {
  ...
  console.log('\n📋 【3】使用 MetaID-Agent 发送 Buzz（内容为上一步 LLM 输出）')
  ...
  try {
    const metaidPath = path.join(__dirname, '..', '..', 'MetaID-Agent', 'scripts')
    ...
    console.error('❌ 无法加载 MetaID-Agent (buzz/wallet)，请确保 MetaID-Agent 在 ../MetaID-Agent')
    process.exit(1)
  }
